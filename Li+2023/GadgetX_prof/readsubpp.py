...
def readsubid(sn, crn, pt, ptype):
    ...
    file = Xspath + 'NewMDCLUSTER_0%03d/snap_%03d' %(crn, sn)
    gid = readsnap.readsnapsgl(file , block = 'ID  ', ptype=pt)

    filegal = glob.glob('/home2/weiguang/The300/data/catalogues/AHF/GadgetX/%s/GadgetX-%s.snap_%03d.z*.AHF_halos' %(cn,cn,sn))[0]
    filesubid = glob.glob('/home2/weiguang/The300/data/catalogues/AHF/GadgetX/%s/GadgetX-%s.snap_%03d.z*.AHF_particles' %(cn,cn,sn))[0]
