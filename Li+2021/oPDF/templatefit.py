...
def templatefit(fitmodel, model, pp, rin,rout, nubtype, nbin):
	...
	if fitmodel == 'TMP':
		filepp = '/home/qyli/oPDFnew/data/particle/%s_%s_fitmc_rcin%s_%s_TMP.txt' %(model,pp,rin,nubtype)
	else:
		filepp = '/home/qyli/oPDFnew/data/particle/%s_%s_fitmc_rcin%s_%s.txt' %(model,pp,rin,nubtype)
	...
	np.savetxt('/home/qyli/oPDFnew/data/TMPMasspro/%s_%s_rin%s_rout%s_%s_%s_M.txt' %(model,pp,rin,rout,nubtype,fitmodel),template_M)

templatefit(fitmodel = 'NFW',model = 'GXsub', pp = 'DM', rin = 0, rout = 1, nubtype = '100000', nbin = 20)
