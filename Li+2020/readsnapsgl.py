...
def readsnapsgl(filename, block, endian=None, quiet=False, longid=False, nmet=11,
    ...
    if block == 'IDTP':  # Particle type
        ...
        if fmt >= 0:
            ...
            if block == "MASS":
                idg0 = (npart > 0) & (masstbl <= 0) # judge 0,out of Ture or Falsw
                if fullmass:
                    if len(npart[idg0]) == 0:  # No Mass block! the number of ture
                        idg1 = (npart > 0) & (masstbl > 0)
                        if len(npart[idg1]) == 1:
                            return masstbl[idg1] #return the masstbl which the number != 0
                        else:  # multi masstble
...
def read_block(npf, block, endian, quiet, longid, fmt, pty, rawdata):
    global nmets
    endf = fstat(npf.fileno()).st_size # the size of the data
    ...
    while npf.tell() < endf:  # End of file   get the location
        if fmt != 0:
            bname, bsize = read_bhead(npf)
            bsize = npf.read(4)
...
#a = readsnapsgl(filename = '/home/qyli/Documents/thesis/data/snp4/GadgetX/snap_128', block = 'Z   ', endian=None, quiet=False, longid=False, nmet=11,
 #               fullmass=False, mu=None, rhb=True, fmt=None, ptype=0, rawdata=False)
#print(a)
#print(a.shape)
