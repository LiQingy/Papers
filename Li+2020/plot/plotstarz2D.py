...
def main(phy,rscale,savepic):
	...
	fileBCG = '/Users/liqy/Documents/data/300data/AHF/AHFdata/GX_snap_metalBCG_r%s.txt' %rscale
	GX = np.loadtxt(file) 
	GXone = np.loadtxt(fileone) 
	GXBCG = np.loadtxt(fileBCG) / 0.0134
	GX = GX / 324 / binare 
	...
	fileBCG = '/Users/liqy/Documents/data/300data/AHF/AHFdata/GM_snap_metalBCG_r%s.txt' %rscale
	GM = np.loadtxt(file) 
	GMone = np.loadtxt(fileone) 
	GMBCG = np.loadtxt(fileBCG) / 0.0134
	GM = GM / 324 / binare 

	print(np.min(SAG[SAG > 0]), np.max(SAG))
	print(np.min(SAGE[SAGE > 0]), np.max(SAGE))
	print(np.min(SDSS7[SDSS7 > 0]),np.max(SDSS7))
	print(np.min(GX[GX > 0]),np.max(GX))
	print(np.min(GM[GM > 0]),np.max(GM))
	print(SAGBCG.shape,SAGEBCG.shape,GXBCG.shape,GMBCG.shape,SDSS7BCG.shape)
	...
	bin1 = 30
	binr = (0,3)
	lvf = np.logspace(np.log10(1e-4),np.log10(1000),100)
	mapp = 'jet'
	...
	# ax30.axes.set_xticks([300,600,900])
	...
	ax50.hist(SDSS7one,bins = bin0,range = binr,histtype = 'step',orientation='horizontal',linewidth = lwd,color = cSDSS7,label = 'Satellite')
	...
	# ax50.axes.set_xticks([50,100,150])
	...
	# ax10.axes.set_xticks([400,800,1200])
	...
	# ax20.axes.set_xticks([500,1000,1500])
	...
	for sym in axlist1:
		sym.set_ylabel(ylab,fontsize = 15, fontweight = 800)
	...
	fig.tight_layout()
	cbar = fig.colorbar(SDSS7p, ax = axlist,ticks = [1e-4,0.001,0.01,0.1,1,10,100,1000],shrink = 0.8)
...
main(phy = 'starz2D',rscale = 0.015,savepic = True)