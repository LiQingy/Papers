...
def G_CCandNCC(ax1):
	...
	def fG(i,xin,xout,A,B,sigamint,ll, lab):
		xx = np.linspace(xin,xout,150)
		yy = A * xx**B * np.exp(sigamint)
		if i == 0:
			return ax1.plot(xx,yy,c = 'k',ls = ll, lw = 2.5, label = lab,zorder = 100)
		else:
			return ax1.plot(xx,yy,c = 'k',ls = ll, lw = 2.5,zorder = 100)
	#CC
	ll = '-'
	lab = 'Ghirardini + 2019 (CC)'
	xx0 = np.array([0.02,0.22,0.66])
	xx1 = np.array([0.19,0.66,1.57])
	A = np.array([1.4,0.72,0.66])
	B = np.array([0.19,-0.23,-0.22])
	sigamint = np.array([0.04, 0.07, 0.08])
	for i in range(3):
		fG(i,xx0[i],xx1[i],A[i],B[i],sigamint[i],ll,lab)
	#NCC
	ll = '--'
	lab = 'Ghirardini + 2019 (NCC)'
	xx0 = np.array([0.01,0.28,0.69])
	xx1 = np.array([0.27,0.68,1.90])
	A = np.array([1.13,0.68,0.64])
	B = np.array([0.05,-0.31,-0.31])
	sigamint = np.array([0.17, 0.15, 0.13])
	for i in range(3):
		fG(i,xx0[i],xx1[i],A[i],B[i],sigamint[i],ll,lab)
		...
		if rel == 'state':
			if raxn == 1:
				...
				if col == 'red':
					...
					pp0 = (0.7,0.66,0.02,4,0.34,0.55)#better one
			...
			if raxn == 1:
				if col == 'red':
					...
					pp0 = (0.68,0.64,0.04,2,0.42,0.55) #done、
					coeffs, matcov = curve_fit(func2, x0, y0, p0 = pp0)
					# coeffs = [0.6,1.18,0.8,0.6,0.44,0.44]
			else:
				if col == 'red':
